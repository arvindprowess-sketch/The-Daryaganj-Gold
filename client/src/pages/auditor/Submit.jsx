  ...
  return (
    <div className="min-h-full pb-28">
              ...
              {uncounted.map((i) => (
                <div key={i.id} className="flex items-center justify-between px-4 py-3">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">{i.code} · {i.unit}</div>
                  </div>
      ...
      <div className="fixed bottom-0 inset-x-0 p-3 bg-gradient-to-t from-slate-100 to-transparent">
        <button className="btn-primary w-full" disabled={uncounted.length > 0}
                onClick={() => setDone(true)}>
          {uncounted.length > 0 ? `${uncounted.length} item(s) remaining` : 'Submit audit count'}
        </button>
      </div>
    </div>
  );
